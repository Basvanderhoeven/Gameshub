...
class Cronjob extends Controller
{
    public function updatestock(){
       $games = $this->game->getGameList();
       foreach($games as $game){
           $this->stock->setFysical($game['id'], $game['sellable']);
       }
    }
    public function generatepreorder(){
        ...
        $orderrows = $this->order->getAllOpenPreOrders();
        var_dump($orderrows);
        foreach($orderrows as $orderrow){
            $game = $this->game->getAvailableGameById($orderrow['game_id']);
            var_dump($game);
            if($game){
            $order = $this->order->getOrderById($orderrow['order_id']);
            $customer = $this->customer->getCustomerById($order['customer_id']);
            //$this->order->resetPreorder($orderrow['id']);
            
            $this->generatePreOrderPDF($customer, $orderrow, $game, $order);
            }
            
        }
    }
    public function generatePreOrderPDF($customer, $orderrow, $game, $order){
        var_dump($game);
        $table_row ='<tr>'
                . '<td>'.$game['name'].'</td>'
                . '<td>'.$game['platform'].'</td>'
                ...
                . '</tr>';
        $total_price = $game['price'] * $orderrow['amount'];
            ...
            $mail->Body = "Bedankt voor uw preorder bij Gameshub, in de bijlage vind u een pdf met daarin de ophaalbon. print deze uit en neem deze mee naar de winkel om uw game(s) op te halen";
        if(!$mail->Send())
            {
